...
@GetMapping({"", "/"})
public String viewAllJobs(@RequestParam(defaultValue = "0") int page, Authentication auth, Model model) {
    if (auth != null && auth.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("EMPLOYER"))) {
